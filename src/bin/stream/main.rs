...
use whisper_stream::{
    model::*, token::Gpt2Tokenizer, token::Language, transcribe::waveform_to_text,
};
...
fn main() {
    ...
    std::thread::spawn(move || record_audio(sender1));
...
fn parse_args() -> (String, Language) {
    ...
    if args.len() < 2 {
        eprintln!("Usage: {} <model name> <lang>", args[0]);
        process::exit(1);
    }
...
fn process_audio_data(
    ...
    for (i, _) in iter::repeat(()).enumerate() {
        ...
        //RUN INFERENCE
        let speech_segment_f32: Vec<f32> = audio_data_vectors
            .clone()
            .into_iter()
            .map(|x| x as f32 / 32767.0)
            .collect();
        let start_time = Instant::now(); // Capture the start time
        let text = match waveform_to_text(&whisper, &bpe, lang, speech_segment_f32, 16000, true) {
    ...
    let stream = device
        .build_input_stream(
            ...
            move |data: &[f32], _: &cpal::InputCallbackInfo| {
                ...
                // Normalize f32 samples (-1.0 to 1.0) to i32 range (-32768 to 32767)
                let vad_data_i16_16k: Vec<i16> =
                    data_16k.iter().map(|x| (*x * 32767.0) as i16).collect();
